...
public class HttpUtils {
    ...
    private void init(String keyStorePath, String keyStorePass) {
        ...
        String[] patterns = {"EEE, dd MMM yyyy HH:mm:ss Z"};
        CookieSpecProvider easySpecProvider = (HttpContext context) -> new DefaultCookieSpec(patterns, false) {
            @Override
        ...
        requestConfig = RequestConfig.custom().setSocketTimeout(MAX_SOCKET_READ_TIMEOUT)
    ...
    private RequestWrap create(HttpRequestBase request, HttpUtils httpUtils) {
        return new RequestWrap(request, cookieStore, objMapper, httpUtils);
    }
    ...
    public ResponseWrap execute(RequestWrap request) {
        ...
        try {
            ...
            context.setCookieStore(cookieStore);
            request.getRequest().setConfig(requestConfig);
            CloseableHttpResponse response = httpClient.execute(request.getRequest(), context);
            return new ResponseWrap(request.getRequest(), response, context, objMapper);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
